...
export function TripDashboardPage({ tripId }: { tripId: string }) {
  ...
  return (
    <AppShell tripId={tripId}>
      {/* Trip hero */}
      <section className="card tripHero">
        <div>
      ...
      <div className="stats">
        <div className="card stat">
          <span className="statHeader"><span className="muted" style={{ fontSize: 12 }}>Total expense</span><span className="statIcon blue"><WalletCards size={16} /></span></span>
          <b>{formatMoney(total, bundle.trip.currency)}</b>
        </div>
        <div className="card stat">
          <span className="statHeader"><span className="muted" style={{ fontSize: 12 }}>Members</span><span className="statIcon violet"><Users size={16} /></span></span>
          <b>{bundle.members.length}</b>
        </div>
        <div className="card stat">
          <span className="statHeader"><span className="muted" style={{ fontSize: 12 }}>Should receive</span><span className="statIcon green"><TrendingUp size={16} /></span></span>
          <b>{receivers.length}</b>
        </div>
        <div className="card stat">
          <span className="statHeader"><span className="muted" style={{ fontSize: 12 }}>Need to pay</span><span className="statIcon orange"><TrendingDown size={16} /></span></span>
          <b>{payers.length}</b>
        </div>
      </div>

      {/* Two-column: spending + recent expenses / balances + settle plan */}
      <div className="twoCol" style={{ alignItems: "start" }}>
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div className="card expenseInsightCard">
            ...
            {bundle.expenses.length ? (
              ...
              <div className="empty">
                <div><h3>No graph yet</h3><p className="muted">Add expenses to see the trip spending graph.</p></div>
              </div>
            )}
          </div>
        ...
        <aside style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div className="card">
            <h2 style={{ marginBottom: 12 }}>Who owes money</h2>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {payers.length ? payers.map((item) => (
                <div className="row" key={item.member.id}>
                  <span style={{ fontFamily: "var(--font-display)", fontWeight: 600, fontSize: 14 }}>{item.member.name}</span>
                  <span className="badge owe">owes {formatMoney(Math.abs(item.balance), bundle.trip.currency)}</span>
                </div>
              )) : <p className="muted">You are all settled.</p>}
            </div>
          </div>

          <div className="card">
            <h2 style={{ marginBottom: 12 }}>Settlement plan</h2>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {settlementDrafts.length ? settlementDrafts.map((draft) => {
                ...
                return (
                  <p className="muted" key={`${draft.from_member_id}-${draft.to_member_id}-${draft.amount}`}>
                    {from?.name} owes {to?.name} {formatMoney(draft.amount, bundle.trip.currency)}
                  </p>
                );
